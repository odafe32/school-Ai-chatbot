            ...
            <div class="w-64 bg-white border-r border-gray-200 flex flex-col transition-all duration-300 ease-in-out z-50 fixed md:relative h-full transform -translate-x-full md:translate-x-0" id="sidebar">
                ...
                <div class="p-4 border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('chat.new') }}">
                        @csrf
                        <button type="submit" class="w-full px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors duration-200 flex items-center gap-2">
                    ...
                    <div id="chatHistory" class="space-y-1">
                        @if($recentChats && count($recentChats) > 0)
                            @foreach($recentChats as $chat)
                                <div class="chat-item {{ $currentChat && $currentChat->id == $chat->id ? 'active bg-blue-600' : 'hover:bg-gray-50' }} p-3 rounded-lg cursor-pointer text-sm transition-colors duration-200 group" 
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            @if($chat->latestMessage)
                                                <div class="text-xs {{ $currentChat && $currentChat->id == $chat->id ? 'text-blue-100' : 'text-gray-500' }} truncate mt-1">
                                                    {{ Str::limit($chat->latestMessage->content, 23) }}
                                                </div>
                                            @endif
                                            <div class="text-xs {{ $currentChat && $currentChat->id == $chat->id ? 'text-blue-200' : 'text-gray-400' }} mt-1">
                                        ...
                                        <div class="opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex-shrink-0">
                                            <button onclick="event.stopPropagation(); deleteChat({{ $chat->id }})" 
                                                    class="p-1 {{ $currentChat && $currentChat->id == $chat->id ? 'text-blue-200 hover:text-white' : 'text-gray-400 hover:text-red-500' }} transition-colors duration-200">
                ...
                <div class="px-4 py-4 bg-white border-t border-gray-200">
                    <div class="max-w-4xl mx-auto">
                        <form method="POST" action="{{ route('chat.send') }}" id="chatForm">
                            @csrf
                            <div class="relative bg-gray-100 border border-gray-300 rounded-xl transition-all duration-200 focus-within:border-blue-500 focus-within:shadow-lg focus-within:shadow-blue-500/10">
                                <textarea 
                                    name="message"
                                    id="messageInput"
                                ...
                                <button 
                                    type="submit"
                                    id="sendButton"
                                    class="absolute right-2 top-1/2 transform -translate-y-1/2 w-8 h-8 bg-blue-600 border-none rounded-lg text-white cursor-pointer flex items-center justify-center transition-all duration-200 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
